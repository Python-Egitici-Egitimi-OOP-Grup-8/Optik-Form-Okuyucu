...
def girisVeTokenAl(kullaniciAdi, parola):
    kullaniciGirisBilgileri["username"] = kullaniciAdi
    kullaniciGirisBilgileri["password"] = parola
    
    # print("sunucu işlemlerinden", kullaniciGirisBilgileri)
    
    headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
    r = requests.post(url = Giris_API, data=json.dumps(kullaniciGirisBilgileri), headers=headers)
    ...
    if r.status_code == 201:
        ...
        kullaniciToken["token"] = token["token"]        
        print(kullaniciToken)
    ...
    with open(csvDosyaYolu) as csv_file:
        ...
        for row in csv_reader:
            if (len(row)!=0 and line_count!=0):
                ...
                for j in range(6,56):
                    if row[j]!="":
                        data_set["C"+str(j-5)]=harfToSayi(row[j])
...
def veriGonder(data):
    ...
    headers = {'Content-type': 'application/json','Authorization': 'Token ' + kullaniciToken["token"]}
